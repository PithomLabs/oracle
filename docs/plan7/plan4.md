...
**Date:** 2026-09-18
**Status:** Draft v4 — Implementation-ready (all blockers resolved)
**Scope:** Developer experience reproducibility for ARGUS Phase 8 dry run
**Reviews applied:** docs/plan7/plan_review.md + 5 final corrections + 3 blockers + 2 corrections
...
| # | Issue | v3 Defect | v4 Fix |
|---|-------|-----------|--------|
| 19 | Conductor credential mismatch | `CONDUCTOR_API_KEY` inconsistent across targets | Single `CONDUCTOR_API_KEY: "argus-operator:operator-001"` variable used everywhere |
| 20 | Trust UI address malformed | `status:trust-ui` used `{{.COORDINATOR_ADDR}}:8081` | Introduced `TRUST_UI_ADDR: "localhost:8081"` |
| 21 | `task dev` destructive | Second invocation wipes DB via `task setup → db:reset` | Split: `task dev` = safe/repeatable, `task fresh` = destructive reinit |
| 22 | Operator token conflated | `ARGUS_OPERATOR_TOKEN` reused `SOLVENT_API_KEY` | Three separate credentials: `SOLVENT_API_KEY`, `CONDUCTOR_API_KEY`, `ARGUS_OPERATOR_TOKEN` |
| 23 | MCP interface misleading | `Run(ctx, handler http.Handler, url)` | `Run(ctx, client *http.Client, url) error` — pure HTTP client |
| 24 | Dependency SHA immutability | Solvent pinned to tag only | Documented: Solvent `v0.1.0` is lightweight tag (immutable); Conductor uses SHA |
...
git clone <oracle-repo>
    ↓
task dev              # safe, repeatable: deps + build + ensure DB + up + status
    ↓
    → CockroachDB READY
    → Solvent READY
    → Conductor READY
...
`task dev` is safe to run repeatedly — it never destroys existing state.
`task fresh` performs destructive reinitialization (wipes DB + rebuilds).

| Command | DB reset? | Use case |
|---------|-----------|----------|
| `task dev` | No | Daily development, iterative work, first-time onboarding |
| `task fresh` | Yes | Clean POC reproduction, wipe-and-rebuild |
| `task setup` | Yes | Full initialization (called by `task fresh`) |
| `task db:reset` | Yes | Explicit DB wipe only |
...
```go
func Run(ctx context.Context, client *http.Client, coordinatorURL string) error {
    ...
    server.AddTool(&mcp.Tool{
        ...
        // HTTP GET to Coordinator using injected client
    })
    ...
    server.AddTool(&mcp.Tool{
        ...
        // HTTP POST to Coordinator using injected client
    })
...
func main() {
    ...
    resp, err := client.Get(coordinatorURL + "/packs/rules")
    if err != nil || resp.StatusCode != 200 {
        log.Fatalf("Coordinator unreachable at %s", coordinatorURL)
    }

    // Pass the same client to MCP server — it's an HTTP client, not a handler
    if err := mcpserver.Run(context.Background(), client, coordinatorURL); err != nil {
...
- **Solvent `v0.1.0`**: Lightweight tag pointing to a specific commit SHA. Lightweight tags are immutable in Git — once created, the tag cannot be moved. This is acceptable for reproducibility. If stronger guarantees are needed, resolve the tag to its SHA (`git rev-parse v0.1.0`) and document it.
- **Conductor `c243f27`**: Commit SHA — inherently immutable. Conductor has no release tags; commit SHA is the strongest pin available.
...
dev:
  ...
  cmds:
    - task: setup:check      # verify prerequisites
    - task: setup:dirs       # create dirs (no-op if exists)
    - task: setup:deps       # clone/update pinned deps (idempotent)
    - task: setup:build      # build all binaries
    - task: setup:config     # generate .opencode/config.json
    - task: up               # start services + ensure DB + readiness
    - task: status           # report actual readiness
```

`task dev` never destroys existing state. It reuses existing binaries,
existing dependencies, and existing database. Safe to run after
`task dev` — the second invocation starts services and reports status.
...
fresh:
  ...
  cmds:
    - task: setup            # full destructive init (includes db:reset)
    - task: up
    - task: status
```

`task fresh` = `task setup` (destructive) + `task up` + `task status`.
Use this for clean POC reproduction.
...
└── task up
    ├── db:up + db:wait
    ├── db:ensure              ← NEW: non-destructive DB check
    ├── up:solvent
    ├── up:conductor
...
├── task setup (includes db:reset)
├── task up (includes db:ensure — skipped after db:reset)
└── task status
```
...
```yaml
vars:
  SOLVENT_API_KEY: "argus-operator"                    # Solvent Bearer token
  CONDUCTOR_API_KEY: "argus-operator:operator-001"    # Conductor X-API-Key (key:actorID)
  ARGUS_OPERATOR_TOKEN: "argus-operator-decision-001"  # Coordinator /decisions auth
  SOLVENT_OPERATOR_ID: "00000000-0000-0000-0000-000000000001"
```

Three separate credentials with distinct roles:
- `SOLVENT_API_KEY` → Solvent service authentication
- `CONDUCTOR_API_KEY` → Conductor service authentication
- `ARGUS_OPERATOR_TOKEN` → Coordinator human-decision authentication
...
up:
  ...
  cmds:
    ...
    - task: db:ensure                    # non-destructive: create DB + migrate IF needed
...
### `db:ensure` (non-destructive)

```yaml
db:ensure:
  desc: "Ensure Solvent DB exists and is migrated (non-destructive)"
  cmds:
    # Check if fable database exists and has the schema_migrations table
    - |
      exists=$$(docker exec {{.CRDB_CONTAINER}} cockroach sql \
        --url "postgres://root@localhost:26260/defaultdb?sslmode=disable" \
        -e "SELECT count(*) FROM [SHOW DATABASES] WHERE database_name = 'fable'" \
        --format csv 2>/dev/null | tail -1 || echo "0")
      if [ "$$exists" = "1" ]; then
        echo "Solvent DB exists, skipping migration"
        exit 0
      fi
      echo "Solvent DB missing, running Solvent db:reset"
      exit 1  # triggers fallback to setup:db
  preconditions:
    - sh: docker exec {{.CRDB_CONTAINER}} cockroach sql -e "SELECT 1" 2>/dev/null
      msg: "CockroachDB not running"
```
...
up:conductor:create-project:
  ...
  cmds:
    - |
      resp=$$(curl -sf -H "X-API-Key: {{.CONDUCTOR_API_KEY}}" \
...
| Service | Probe | Auth | Method | Timeout | Retry |
|---------|-------|------|--------|---------|-------|
| CockroachDB | `docker exec ... cockroach sql -e "SELECT 1"` | none | Shell | 60s | 1s × 60 |
| Solvent | `GET /v1/ledger?scenario_id=track1` | Bearer `SOLVENT_API_KEY` | HTTP | 30s | 1s × 30 |
| Conductor | `GET /v1/projects` | X-API-Key `CONDUCTOR_API_KEY` | HTTP | 30s | 1s × 30 |
| Coordinator | `GET /packs/rules` | none | HTTP | 30s | 1s × 30 |
| Trust UI | `GET /insights` | none | HTTP | 10s | 1s × 10 |
...
status:conductor:
  cmds:
    - |
      if [ -f {{.PIDS}}/conductor.pid ] && kill -0 $$(cat {{.PIDS}}/conductor.pid) 2>/dev/null; then
        code=$$(curl -sf -o /dev/null -w '%{http_code}' -H "X-API-Key: {{.CONDUCTOR_API_KEY}}" \
...
status:trust-ui:
  cmds:
    - |
      if [ -f {{.PIDS}}/trust-ui.pid ] && kill -0 $$(cat {{.PIDS}}/trust-ui.pid) 2>/dev/null; then
        code=$$(curl -sf -o /dev/null -w '%{http_code}' \
          http://{{.TRUST_UI_ADDR}}/insights 2>/dev/null || echo "000")
...
```text
task fresh   # clean POC reinitialization (wipes DB, rebuilds everything)
task test    # verify unit tests pass
task verify  # verify physics verifier runs
```

**Use `task fresh` for the dry run** — requires clean epistemic state.
Use `task dev` for iterative development (safe, non-destructive).
...
- [ ] `task dev` is safe to run repeatedly (never destroys DB)
- [ ] `task fresh` = destructive reinit (DB wiped, rebuilt, all READY)
- [ ] `task dev` ≠ `task setup` — `task setup` includes `db:reset`, `task dev` does not
...
- [ ] All readiness checks pass (HTTP probes with correct auth, no sleep-only)
...
- [ ] Conductor process uses `CONDUCTOR_API_KEY` (not `SOLVENT_API_KEY`)
- [ ] Coordinator uses `CONDUCTOR_API_KEY` for Conductor client (not `SOLVENT_API_KEY`)
- [ ] `up:conductor:create-project` uses `CONDUCTOR_API_KEY`
- [ ] `ARGUS_OPERATOR_TOKEN` is separate from `SOLVENT_API_KEY`
- [ ] Trust UI starts with `ARGUS_OPERATOR_TOKEN` (not `SOLVENT_API_KEY`)
- [ ] Trust UI status probe uses `TRUST_UI_ADDR` (not `{{.COORDINATOR_ADDR}}:8081`)
- [ ] Conductor project creation fails hard on error (no `|| true`)
- [ ] Dry-run instructions reference `task fresh` (not `task dev`)
...
- [ ] No `http.Handler` in MCP `Run` signature
- [ ] No `{{.SOLVENT_API_KEY}}` used as Conductor credential
...
| `db:ensure` heuristic may miss incomplete migrations | If DB exists but schema is partial, `task fresh` is the recovery path |
| `task dev` does not guarantee DB schema is current | If schema drifts, `task fresh` reinitializes cleanly |
