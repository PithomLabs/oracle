...
**Date:** 2026-09-18 (revision 4 — final before implementation)
...
### Step K: Resolve pack from PackRef, not hardcoded "bmist"

**File:** `internal/application/app.go`

Add `PackRef` to `DecisionRequest`:
```go
...
Add resolver helper:
```go
func (a *App) resolvePack(packRef string) (domainpack.Pack, error) {
    if a.packRegistry == nil {
        return nil, fmt.Errorf("pack registry unavailable")
    }
    if packRef == "" {
        return nil, fmt.Errorf("pack_ref is required for pack resolution")
    }
    packID, packVersion := parsePackRef(packRef)
    return a.packRegistry.Get(packID, packVersion)
}

func parsePackRef(ref string) (string, string) {
    // Support both "bmist-1.0.0" and "bmist@1.0.0" formats
...
### Step L: Full retirement-rule enforcement in SubmitDecision
...
case "discharge":
    ...
    pack, err := a.resolvePack(req.PackRef)
    if err != nil {
...
### Step N: Update Trust UI to send PackRef

**File:** `internal/ui/ui.go` — `HandleDischarge` adds `PackRef` field:
...
### CRDB startup with EADDRINUSE retry — FAIL CLOSED
        ...
        for attempt in 1 2 3; do
          ...
          for i in $(seq 1 30); do
            ...
            if ! kill -0 $CRDB_PID 2>/dev/null; then
              break  # CRDB exited — port conflict
            fi
            sleep 1
          done
...
- **`internal/application` does NOT hardcode "bmist".** Pack resolved from `PackRef`.
