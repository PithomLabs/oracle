---
title: "The Elephant Bridge Protocol and the BM–IST–Asymptotic Safety Research Program — v1.1"
...
\text{relational clock / effective foliation / covariant equivalent}.
$$

The strongest target is not merely a hidden preferred slicing. It is IR equivalence under admissible clock or foliation choices: different microscopic representations may be allowed to organize updates differently, while physical observables become the same in the relativistic regime. This remains a downstream recovery test, not a microscopic axiom.
...
# 10. The Candidate Flow Architecture: Three Regimes

The candidate architecture is summarized by

$$
\boxed{
X
\xrightarrow{\text{microscopic dynamics}}
\text{controlled coarse-graining}
\xrightarrow{\text{candidate effective/RG flow}}
\mathcal T_{\rm IR}
...
This is deliberately **not** a claim that one unified flow has already been constructed. The existence and faithfulness of the common flow remain a central theorem target.
...
The synthesis's deepest unresolved object is not a new particle. It is a family of typed bridges.

The program distinguishes

$$
X=\text{microscopic state space},\qquad
\mathcal M(X)=\text{measures on }X,\qquad
\mathcal T=\text{effective-action/theory space},\qquad
\mathcal O=\text{observable space}.
$$

A microscopic update acts on states, an RG map acts on theory descriptions, and an observable projection acts toward what can actually be measured:

$$
F:X\to X,\qquad
\mathfrak R_s:\mathcal T\to\mathcal T,\qquad
\Pi:X\to\mathcal O.
$$

The state-to-theory bridge is a separate object:
...
$$
X_L\xrightarrow{\mathcal C_s}X_{L,s}\xrightarrow{\mathcal A_s}\mathcal T_s,\qquad
\Phi_s=\mathcal A_s\circ\mathcal C_s.
$$

The original projection problem is therefore sharpened into a typed construction problem rather than a single unspecified map.
...
### 11.1 The complex amplitude factor is a load-bearing gate

The latest adversarial analysis sharpens the main bottleneck. It is not enough to obtain a smooth density, write Madelung equations, or invoke Fisher information. The microscopic dynamics must generate a genuine complex amplitude sector that is closed under evolution.

A candidate route is a nontrivial tame/Kronecker factor
...
where the factor supports a nontrivial unit-circle character structure. Native arithmetic characters such as

$$
\chi_p(x)=e^{2\pi i\{x\}_p}
$$

are workbench hypotheses, not promoted answers. The required ladder is:

1. invariant measure and transfer/Koopman objects exist;
2. a nontrivial tame/Kronecker factor survives in the dynamical limit;
3. the factor supports the required phase structure;
4. the phase and modulus close into a physically adequate complex amplitude and generator.

If the limiting dynamics are weakly mixing with no viable tame factor and no independently defensible alternative amplitude construction, the amplitude route for that substrate is a hard failure.
...
**Speculative structural hypotheses:** arithmetic organization of the UV, a measure-level rather than set-level fractal signature, scale-dependent spectral dimension, cross-place correlation structure, a tame/Kronecker route to the complex amplitude sector, and an E8-coded information-sector possibility.

**Explicitly rejected or demoted claims:** literal identification of state-space invariant sets with theory-space stable manifolds; automatic identification of nonlocality as an IR artifact; naive E8 chirality rescue; and the idea that 248 is a count of physical particles. The current program is self-critically revised, but external red-team validation remains pending.
...
The immediate center of gravity is a single frozen microscopic test substrate. The first physical construction is the smallest pre-registered arithmetic class that can support the required algebraic and dynamical tests; v6.3.1 begins with a globally algebraic degree-2 self-map with an explicit $p=2$ local realization.

### Deliverable A — Frozen microscopic test substrate

State explicitly:

$$
X_L,\quad L,\quad\text{encoding},\quad F_L,\quad\text{placewise action},
$$

plus boundary/initial conventions and the allowed parameter set. Once complete, freeze this object. A downstream result may not modify $F_L$ or its declared conventions to improve a result. A changed substrate is a new candidate and restarts the dependent tests.

Deliverable A is **not** a promoted physical claim. It is the experimental specimen against which the next tests are run.

### Deliverables B–F — Run on the same frozen substrate

- canonical-height applicability, including exact hypotheses;
- Markov/symbolic applicability on the same map;
- transfer/Koopman and tame-factor diagnostics;
- prime-universality and p-adic-character closure;
- a non-arithmetic null substrate of comparable finite complexity.

B–F may be developed in parallel as mathematical methods, code, and analysis tooling. Their scientific evidence becomes meaningful only after A is frozen.

The sequencing matters. A beautiful E8 story should not outrun the microscopic map. A spectral-dimension curve should not compensate for a missing invariant measure. A cosmological prediction should not conceal post-hoc substrate choices. Likewise, the recent thought experiments—black-hole information, vacuum/cosmological-constant response, preferred foliation and relational time, Big Bang/arrow of time, inflation/primordial spectra, and galactic IR gravitational anomalies—should remain **downstream stress tests of the same derived machinery**, not new foundations.

The governing principle is unchanged:

$$
\boxed{\text{Construct the substrate before interpreting its possible consequences.}}
$$
