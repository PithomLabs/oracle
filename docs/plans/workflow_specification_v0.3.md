...
### 7.0 Effect-Capable Integration Declaration

Each effect-capable integration MUST maintain a versioned declaration of its consequential operations and boundary behavior.
...
The declaration does not create authority. Solvent remains the authority owner.
...
The Executor MUST compare the requested operation against the operation bound to the authorization evidence.
...
An UNKNOWN / UNAVAILABLE result MUST NOT be reinterpreted as DENIED.
...
Humans may still intervene unexpectedly or where external requirements/explicit policy require participation.
...
- authorization evidence carried by/directly bound to the authorization result
...
## 29. Conformance Readiness Gate

Before this specification is considered a stable basis for implementation, the following MUST be demonstrable:

1. A production integration can be tested for mismatched operation rejection.
2. The authorization evidence carries or directly exposes the exact authorized operation binding.
3. Integration declarations are versioned and re-conformance-triggering.
4. UNKNOWN authorization has a declared unresolved-wait/age policy.
5. Long-running authority validity is declared and exercised.
6. Actual integration negative testing is possible in a safe/reversible environment.
7. Scenarios explicitly cover long-running execution and phase/persistence inspection.
8. The conformance harness can deterministically observe every fact required by its pass/fail assertions.
