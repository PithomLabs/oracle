# Loop Engineering Role / Boundary Matrix v0.5
...
These rows represent two sequential mechanical steps within one logical consequential boundary. They MUST NOT be implemented as independent alternative paths.
...
## 8. Coordination Observation of Consequential Work

Conductor MAY record coordination-relevant facts about a consequential operation using its existing lifecycle/activity model, including that work is:

- awaiting an external authorization decision
- awaiting an external execution result
- awaiting declared reconciliation

Such records are coordination facts only.

They MUST NOT constitute:

- an authority decision
- an authorization state owned by Conductor
- a replacement Solvent state
- proof that an external effect occurred

The execution result MUST be available to:

- the Agent/client for interpretation
- Conductor for coordination/lifecycle observation

Initial result delivery MAY be direct or mediated through applicable interfaces.

## 9. Capability Declaration Lifecycle
...
Where a declared capability has no distinct integration owner, the deployment operator is the default declaration owner for deployment-level conformance and lifecycle accountability.

The deployment operator is responsible for declaring and maintaining the boundary actually deployed, but is not presumed to author the underlying provider/tool semantics.

Where a distinct integration/provider owner exists, that owner remains responsible for the correctness of published capability semantics, while the deployment operator remains accountable for the deployed declaration and conformance scope.
...
The same comparison rule MUST be applied by all participants evaluating operation binding for the same declared operation at proposal, authorization, and execution.

The comparison rule MUST be deterministic and interoperably defined by the declaration rather than inferred independently by each implementation.
...
The applicable declaration owner is responsible for completeness of this definition.

Omitting an effect-relevant parameter is a conformance failure.

Completeness of an operation-identity declaration is an integration/design-time conformance responsibility. The generic protocol harness verifies that declared identity inputs are correctly bound; it does not provide a universal oracle for discovering hidden effect-relevant inputs. Additional integration-specific, domain, or deployment review MAY be required.
...
## 13. Authorization UNKNOWN Temporal Policy
...
An operator-controlled indefinite wait is valid only when an active human operator exists to act on the unresolved hold.

Autonomous operation without such an active operator MUST use a finite or otherwise actionable remediation policy.
...
## 14. Long-Running Execution and Authority Precedence
...
A queue acceptance, message handoff, or intermediate acknowledgement alone MUST NOT be reported as `succeeded` unless the declared outcome model explicitly defines that artifact as authoritative evidence of successful effect occurrence.
...
## 17. Evidence / Correlation Matrix
...
## 18. Persistence / State Ownership
...
## 19. Integration / Capability Declaration

Every declared capability boundary MUST satisfy the canonical capability declaration requirements defined in the declaration lifecycle section above.

For effect-capable capabilities, the declaration additionally governs:
...
Declaration changes that alter effect classification, effect surface, operation identity, or any listed semantic property invalidate prior conformance for the affected boundary until re-conformance completes.

## 20. Production Conformance Boundary
...
## 21. Client / Runtime Substitution Boundary
...
## 23. Conformance Harness Boundary
...
## 26. Matrix-Level Acceptance Criteria
...
16. Conductor can observe coordination-relevant consequential progress without acquiring authority semantics.
17. If no external SOR exists, Executor-as-SOR outcome semantics are explicit.
18. Any proposed exception can be evaluated through the Growth Gate.

## 27. Adversarial Questions for the Conformance Matrix
...
18. If no external SOR exists, is the Executor's authoritative effect evidence sufficient to establish `succeeded`?
19. Can Conductor observe consequential work waiting for authorization/execution without becoming an authority owner?
20. Can an asynchronous handoff be reported as `succeeded` without outcome evidence sufficient under the declared model?
...
| Generic capability owner | Adopted | Deployment accountability without creating a new role |
...
## 29. Precedence and Deliverable Boundary
