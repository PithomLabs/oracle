...
**Version:** 1.1 — Frozen Implementation Baseline
**Date:** 2026-09-15
**Status:** FROZEN — IMPLEMENTATION BASELINE
**Source:** v1.0 execution plan + four blocking fixes + five corrections + consistency pass
...
| Artifact registry | In-memory, read-only for Coordinator. `registerTrusted` is package-private to `verifier/`. |
| Edge creation | Minimal Solvent API endpoint `POST /v1/beliefs/{parent_id}/edges` (Growth Gate exception). No Coordinator SQL. |
| Pack registry | In-memory at `oracle/domain-pack/registry.go`. Coordinator consumes, does not own. |
| Decision persistence | Solvent state + Solvent audit_activity = authoritative. No second DB. |
...
├── domain-pack/                 ← Phase 2
│   ├── registry.go              ← In-memory pack registry
│   ├── registry_test.go
│   └── bmist/v1/
├── packet/v1/                   ← Phase 3
├── verifier/                    ← Phase 4
...
7. **Growth Gate exception:** New minimal Solvent endpoint `POST /v1/beliefs/{parent_id}/edges` for belief-to-belief edge creation. Structural checks: parent exists, child exists, parent ≠ child, kind in {derives, contradicts}, edge uniqueness. This is a minimal kernel extension, not a full API surface. Coordinator writes edges through Solvent's canonical authority boundary; never through direct SQL. Add focused tests. Do not permit arbitrary SQL as a runtime escape hatch.
...
// Register validates then adds a pack. Returns error if invalid or duplicate.
...
// LoadFromDisk scans domain-pack/**, validates each pack.json, registers.
func (r *PackRegistry) LoadFromDisk(root string) error
```

**Invariants:**
- In-memory only, no persistence
- Startup-loaded from `domain-pack/**` via `LoadFromDisk`
- Validated before registration
- Keyed by `pack_id + version`
- No restart recovery (accepted POC limitation)

**Registry tests:**
- Valid pack registers and retrieves
- Duplicate pack_id+version → error
- Invalid pack rejected on registration
- `LoadFromDisk` loads all valid packs from directory tree
- `LoadFromDisk` skips invalid packs with error log
- `Get` with unknown pack_id → error
- `Get` with wrong version → error
- Concurrent registration is safe (race test)
...
// registerTrusted adds a verified artifact. Package-private — only the
// Physics Verifier runner may call it during verification. External code
// cannot register artifacts. This is a compile-time guarantee.
...
- The Physics Verifier runner creates the registry and registers artifacts internally
- The Coordinator only calls `Resolve` and `VerifyHash` (public read methods)
- This makes the architecture match the stated invariant: ONLY trusted verifier output may enter ArtifactRegistry, rather than relying on caller discipline
...
|----------|-----------------|------------------|-------------|
| PROMOTE | `POST /v1/beliefs/{id}/promote` | `belief_promoted` (on success) | Verdict refusal (on debt remaining) |
...
| REFUSE | No mutation | No Solvent audit entry | Verdict from Solvent (if promotion refusal) |
...
4. For each belief in packet:
   ...
   c. For each evidence referencing this belief:
      ...
      - `POST /v1/evidence`
5. For each edge:
   a. Resolve `from_ref`/`to_ref` to canonical belief IDs
   b. `POST /v1/beliefs/{parent_id}/edges` to Solvent with `{child_id, kind}`
   c. Never store epistemic edges only in Coordinator memory
...
- PROMOTE: `POST /v1/beliefs/{id}/promote?scenario_id=X` → On success, Solvent writes `belief_promoted` audit entry. On refusal (HTTP 200 with `Verdict` body), no audit entry is created by Solvent. Coordinator records refusal as `DecisionRecord` with `result: "refused"` and `refusal_reason` from the `Verdict.gate`/`Verdict.reason`.
...
- REOPEN: `POST /v1/beliefs` with same claim + `POST /v1/beliefs/{parent_id}/edges` with `derives` edge from retracted origin → Solvent writes `belief_entered` audit entry.
...
**Edge creation (Growth Gate exception):**
```go
// SolventClient.CreateEdge calls POST /v1/beliefs/{parent_id}/edges
func (c *SolventClient) CreateEdge(ctx context.Context, parentID, childID, kind string) error
```
Endpoint performs structural checks: parent exists, child exists, parent ≠ child, kind valid, edge uniqueness. Coordinator is responsible only for reference resolution; Solvent persists canonical epistemic relationships.
...
- PROMOTE on belief with debt → Solvent returns Verdict (HTTP 200), no audit entry created
- PROMOTE on belief with no debt → Solvent creates `belief_promoted` audit entry
...
- REFUSE (promotion refusal) → no Solvent audit entry, Coordinator records DecisionRecord with `result: "refused"`
...
*Edge creation (Growth Gate exception):*
- Valid edge (derives) created via Solvent API
- Valid edge (contradicts) created via Solvent API
- Self-loop rejected by Solvent
- Duplicate edge rejected by Solvent
- Nonexistent parent rejected by Solvent
- Nonexistent child rejected by Solvent
- Invalid kind rejected by Solvent
- Coordinator resolves `from_ref`/`to_ref` before calling Solvent API
- Edge visible in Solvent belief graph after compilation

*Trust boundary:*
- `registerTrusted` is unexported — compilation fails if called from outside `verifier/` package
- Coordinator cannot register artifacts (only reads from registry)
- Forged artifact not in registry → evidence rejected
...
- PROMOTE on belief with debt → Solvent returns Verdict (no audit entry, refusal recorded by Coordinator)
- PROMOTE on belief with no debt → success, `belief_promoted` audit entry present
- RETRACT on promoted belief with live intent → cascade cancels intent first
- REFUSE (non-promotion refusal) → no Solvent state change, Coordinator records refusal
...
**Dependencies:** Phases 6, 7, 9.1, 9.2.
...
**Dependencies:** Phase 10 (which requires Phase 9.2).
...
```
0 → 1 → 2 → 3 → 4 → 6 → 7 → 8 → 9.1 → 9.2 → 10 → 11 → 12 → 13 → 14 → 15
```
...
POST /v1/beliefs/{parent_id}/edges  → creates belief-to-belief edge (Growth Gate exception)
...
| Solvent API breakage | Pin to frozen API contract (including Growth Gate edge endpoint) |
...
| **Solvent edge endpoint Growth Gate exception** | **Document in freeze reconciliation. Minimal structural checks only. No arbitrary SQL escape hatch.** |
...
8. Solvent edge endpoint (`POST /v1/beliefs/{parent_id}/edges`) is a Growth Gate exception — minimal kernel extension documented in freeze reconciliation.
...
6. Artifact registry enforces trusted-verifier-only admission (registerTrusted is package-private).
...
18. Edge creation goes through Solvent API (Growth Gate exception documented in freeze reconciliation).
