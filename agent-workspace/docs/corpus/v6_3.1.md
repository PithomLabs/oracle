---
title: "BM + IST + Asymptotic Safety — v6.3.1"
...
**A-freeze rule:** once Deliverable A is declared complete, the tuple

$$
\boxed{(X_L,\,L,\,\text{encoding},\,F_L,\,\text{placewise action},\,\text{boundary/initial conventions},\,\text{allowed parameters})}
$$

becomes the frozen Phase-1 test substrate. Downstream results may not be improved by modifying this substrate. Any substantive change to it creates a new candidate substrate and restarts the downstream tests. Deliverable A is a **frozen experimental object, not a promoted physical claim**.
...
**Downstream cross-domain stress tests:** if the core survives its load-bearing gates, the same derived substrate, invariant measure, amplitude sector, state-to-theory bridge, and effective gravitational flow should be tested against black-hole information retention, gravitational vacuum/cosmological-constant response, foliation and relational-time recovery, early-universe dynamics and primordial spectra, and galactic-scale IR gravitational phenomenology. These are downstream tests of the already-derived machinery, not additional microscopic axioms or Phase-1 deliverables.
...
**Deliverable A:** explicit smallest nontrivial $F_L$, then freeze the complete Phase-1 substrate specification.

**Deliverable B:** canonical-height applicability report on that frozen $F_L$.

**Deliverable C:** Markov-partition applicability report on that same frozen $F_L$.

**Deliverable D:** transfer/Koopman diagnostics and first tame-factor/Kronecker-spectrum test.

**Deliverable E:** prime-universality scan and p-adic-character closure test.

**Deliverable F:** explicit non-arithmetic null substrate of comparable finite complexity.

A is the serial dependency for downstream **evidence**, but not a promotion gate: B–F may be developed in parallel as methods and analysis tooling, and they execute on the same frozen $F_L$ once A exists. The expected outcome is not success.
...
- freezes Deliverable A as an experimental substrate rather than treating it as a promoted physical claim;
- reconciles the Phase-1 deliverable numbering with the 90-day plan;
- records the six downstream thought-experiment domains as stress tests rather than new architectural commitments.
